...
class Car(Vehicle):
    ...
    def reserve(self, start_date, days_reserve):
        reserved = True
        print("Vehicle " + self.brand + " " + self.model + " " + self.reg_plate + " " +
              " is reserved for " + str(days_reserve) + " days. Price is " + str(self.price_per_day*days_reserve))

    def getRentalFee(self, days_reserved):
        return self.price_per_day*days_reserved

    def addToDb(self):
        line = [self.brand, self.model, self.reg_plate,
                self.year, self.color, self.mileage, self.car_type]
        with open(self.dbName, mode='a', newline='') as db:
            writer = csv.writer(db)
            writer.writerow(line)
        print("saved in " + self.dbName)
...
class MotorBike(Vehicle):
    ...
    def reserve(self, start_date, days_reserve):
        reserved = True
        print("Vehicle " + self.brand + " " + self.model + " " + self.reg_plate + " is reserved for " +
              str(days_reserve) + " days. Price is " + str(self.price_per_day*days_reserve))

    def getRentalFee(self, days_reserve):
        return self.price_per_day*days_reserve

    def addToDb(self):
        record = [self.brand, self.model, self.reg_plate,
                  self.year, self.color, self.mileage, self.cc]
        with open(self.dbName, mode='a', newline='') as db:
            writer = csv.writer(db)
...
class Document:
    def __init__(self, vehicle, renter):
        self.name = "Agreement-" + vehicle.brand + "-" + vehicle.model + \
            "-" + renter.firstName + "-" + renter.family + ".txt"
    ...
    def generateAgreement(self):
        ...
        f.write("Agreement between Rent a car company and " +
                self.renter_name + " " + self.renter_family + "\n")
        f.write("entered on date " + str(date.today()) + "\n")
        f.write(
            "1. Rental company agrees to rent to the Renter the following vehicle:\n")
        ...
        f.write("The Renter agrees to pay the Rental Company the total sum of " +
                str(self.vehicle_fee) + " leva for the entire rental period.\n")
        f.close()
...
def collectRenterInfo():
    ...
    renter = RentalPerson(firstName, family, nationality,
                          identification_number, card_id, email, phone_number, rent_period)
    return renter
...
def bookVehicle(listname):
    ...
    if action == 0:
        ...
        if listname == "CarsList.csv":
            car = Car(vehicle_info[0], vehicle_info[1], vehicle_info[2],
                      vehicle_info[3], vehicle_info[4], vehicle_info[5], vehicle_info[6])
            doc = Document(car, renter)
            car.reserve(date.today(), renter.rent_period)
        elif listname == "MotorbikesList.csv":
            motorbike = MotorBike(vehicle_info[0], vehicle_info[1], vehicle_info[2],
                                  vehicle_info[3], vehicle_info[4], vehicle_info[5], vehicle_info[6])
